...
from Lib.data_loading import load_and_preprocess, save_segmentation_nifti
from Lib.predict_case import predict_case_3D_net
import imp
from Lib.utils import postprocess_prediction, SetNetworkToVal, get_params_fname, maybe_download_parameters
import os
...
def apply_bet(img, bet, out_fname):
    img_itk = sitkUtils.PullVolumeFromSlicer(util.getNode(img))
    img_npy = sitk.GetArrayFromImage(img_itk)
    # img_bet = sitk.GetArrayFromImage(sitk.ReadImage(bet))
    img_bet = sitk.GetArrayFromImage(sitkUtils.PullVolumeFromSlicer(bet))
    img_npy[img_bet == 0] = 0
    out = sitk.GetImageFromArray(img_npy)
    out.CopyInformation(img_itk)
    sitkUtils.PushVolumeToSlicer(out, name=out_fname)




def run_hd_bet(mri_fnames, output_fnames, m_fname, mode="accurate",
               config_file=os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.py"), device=0,
    ...
    for in_fname, out_fname in zip(mri_fnames, output_fnames):
        ...
        if overwrite or (not (os.path.isfile(mask_fname) and keep_mask) or not os.path.isfile(out_fname)):
            ...
            save_segmentation_nifti(seg, data_dict,keep_mask, output_fnames, mask_fname)
